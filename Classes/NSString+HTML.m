...
- (NSString *)stringByConvertingHTMLToPlainText {
	...
	NSCharacterSet *newLineAndWhitespaceCharacters = [NSCharacterSet characterSetWithCharactersInString:[NSString stringWithFormat:@" \t\n\r%C%C%C", 0x0085, 0x000C, 0x2028]];

	NSCharacterSet *newparCharacters = [NSCharacterSet characterSetWithCharactersInString:[NSString stringWithFormat:@"%C", 0x2029]];
	...
	do {
		...
		if ([scanner scanString:@"<" intoString:NULL]) {
			...
			if ([scanner scanCharactersFromSet:newparCharacters intoString:NULL]) {
			...
			else if ([scanner scanCharactersFromSet:newLineAndWhitespaceCharacters intoString:NULL]) {
...
- (NSString *)flattenHTML {
    ...
    NSScanner *theScanner,*theScanner2;
   // NSString *text = nil;
    theScanner = [[NSScanner alloc] initWithString:self];	
    while ([theScanner isAtEnd] == NO) {
		
        [theScanner scanUpToString:@"<" intoString:NULL] ; 
		
        [theScanner scanUpToString:@">" intoString:&tag] ;
		if ([tag rangeOfString:word1].location != NSNotFound&&[tag rangeOfString:word2].location != NSNotFound&&[tag rangeOfString:word3].location != NSNotFound) {
			//NSLog(@"tag is %@",tag);
			theScanner2 = [[NSScanner alloc] initWithString:tag];
			while ([theScanner2 isAtEnd] == NO) {
				
				[theScanner2 scanUpToString:@"http" intoString:NULL] ; 
				
				[theScanner2 scanUpToString:@"\"" intoString:&url] ;
				//NSLog(@"url is %@",url);
			}
